export const featured = [
  {
    id: 1,
    mainText: 'Transform Your Living Space!',
    smallText: 'Find stylish appliances for modern living.',
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1715694102/ecom/pexels-rdne-5591463_tzqmfq.jpg',
  },
  {
    id: 2,
    mainText: 'Revamp Your Home, Simplify Life!',
    smallText: 'Effortless solutions for everyday tasks.',
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1715694053/ecom/pexels-level23media-3016430_glxn1d.jpg',
  },
  {
    id: 3,
    mainText: 'Upgrade Your Kitchen Essentials!',
    smallText: 'Efficient solutions for culinary perfection.',
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1715694168/ecom/kam-idris-Ot2iTXgC6fY-unsplash_m3j2qv.jpg',
  },
  {
    id: 4,
    mainText: 'Upgrade Your Home Today!',
    smallText: 'Discover top-quality appliances for every room.',
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1715694042/ecom/pexels-houzlook-3423817_lnpiyg.jpg',
  },
]

export const categories = [
  {
    id: 1,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975746/lynxmart/img/category/features_1_rwxy1u.png',
    title: 'Washing Machines',
  },
  {
    id: 2,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975724/lynxmart/img/category/features_2_qvcnfn.png',
    title: 'Phones',
  },
  {
    id: 3,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975724/lynxmart/img/category/features_3_r49tjb.png',
    title: 'Computing',
  },
  {
    id: 4,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975724/lynxmart/img/category/features_4_bkdknd.png',
    title: 'Sound Systems',
  },
  {
    id: 5,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683977004/lynxmart/img/category/Electronics_du6gxe.png',
    title: 'Televisions',
  },
  {
    id: 6,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683976997/lynxmart/img/category/Home_Appliances_m2lhkp.png',
    title: 'Home Appliances',
  },
  {
    id: 7,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683976929/lynxmart/img/category/Beauty_zq4xri.png',
    title: 'Air Conditioners',
  },
  {
    id: 8,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683976985/lynxmart/img/category/Supermarket_rawu2b.png',
    title: 'Refrigerators',
  },
]

export const books = [
  {
    id: 1,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975718/lynxmart/img/products/books/book_1_q40zcq.png',
  },
  {
    id: 2,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975729/lynxmart/img/products/books/book_2_h2tafv.png',
  },
  {
    id: 3,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975720/lynxmart/img/products/books/book_4_ecfcdy.png',
  },
  {
    id: 4,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975716/lynxmart/img/products/books/book_3_zyvm6y.png',
  },
  {
    id: 5,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975716/lynxmart/img/products/books/book_8_eitsea.png',
  },
  {
    id: 6,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975733/lynxmart/img/products/books/book_6_fykdjc.png',
  },
  {
    id: 7,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975735/lynxmart/img/products/books/book_10_b9lcfv.png',
  },
  {
    id: 8,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975720/lynxmart/img/products/books/book_9_j0stlt.png',
  },
  {
    id: 9,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975732/lynxmart/img/products/books/book_5_u3ig32.png',
  },
  {
    id: 10,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975731/lynxmart/img/products/books/book_7_qeqzkd.png',
  },
]

export const brands = [
  {
    id: 1,
    name: 'samsung',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch201_d2fzj5.png',
  },
  {
    id: 2,
    name: 'tcl',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch204_ntcidd.png',
  },
  {
    id: 3,
    name: 'hp',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch208_qhearl.png',
  },
  {
    id: 4,
    name: 'redmi',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch211_zbdm70.png',
  },
  {
    id: 5,
    name: 'lynx',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch207_jvsp85.png',
  },
  {
    id: 6,
    name: 'levi',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch203_qczyyh.png',
  },
  {
    id: 7,
    name: 'adidas',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch209_ucjmpr.png',
  },
  {
    id: 8,
    name: 'pepsodent',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780602/justclick/brands/merch210_kobabp.png',
  },
  {
    id: 9,
    name: 'guniess',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch212_seqgf6.png',
  },
  {
    id: 10,
    name: 'all',
    brandImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683780601/justclick/brands/merch206_1_nhy02z.png',
  },
]

export const blogBanner = {
  title: '#readmore',
  subText: 'Read all case studies about our products!',
  bgImg:
    'https://res.cloudinary.com/khobbylynx/image/upload/v1683975710/lynxmart/img/banner/b99_mn10ou.jpg',
}

export const aboutBanner = {
  title: '#KnowUs',
  subText: 'Learn more about LynxMart!',
  bgImg:
    'https://res.cloudinary.com/khobbylynx/image/upload/v1683975692/lynxmart/img/banner/a1_ysgwwn.png',
}

export const contactBanner = {
  title: "#let's_talk",
  subText: 'Leave a message, We love to hear from you!',
  bgImg:
    'https://res.cloudinary.com/khobbylynx/image/upload/v1682616430/samples/imagecon-group.jpg',
}

export const cartBanner = {
  title: '#cart',
  subText: 'Add your coupon code & SAVE upto 70%!',
  bgImg:
    'https://res.cloudinary.com/khobbylynx/image/upload/v1683975715/lynxmart/img/banner/cartBanner_zoaixl.jpg',
}

export const blogs = [
  {
    id: 1,
    blogImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683975756/lynxmart/img/blog/b1_tb5fpq.jpg',
    title: 'The cotton-Jersey Zip-Up Hoodie',
    desc: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quod iusto est Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus, fugiat. explicabo.',
    date: '13/01',
  },
  {
    id: 2,
    blogImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683975747/lynxmart/img/blog/b2_ocvmba.jpg',
    title: 'How to Style a Quiff',
    desc: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quod iusto est Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus, fugiat. explicabo.',
    date: '13/03',
  },
  {
    id: 3,
    blogImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683975681/lynxmart/img/blog/b3_cgir4i.jpg',
    title: 'Must-Have Skater Girl Items',
    desc: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quod iusto est Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus, fugiat. explicabo.',
    date: '13/06',
  },
  {
    id: 4,
    blogImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683975750/lynxmart/img/blog/b6_zhtgre.jpg',
    title: 'Runway-Inspired Trends',
    desc: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quod iusto est Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus, fugiat. explicabo.',
    date: '12/05',
  },
  {
    id: 5,
    blogImg:
      'https://res.cloudinary.com/khobbylynx/image/upload/v1683975691/lynxmart/img/hero/Hero999px_nt2rlc.png',
    title: 'AW20 Menswear Trends',
    desc: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Quod iusto est Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus, fugiat. explicabo.',
    date: '12/09',
  },
]

export const features = [
  {
    id: 1,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975726/lynxmart/img/features/f4_savtdr.png',
    title: 'Free Shipping',
  },
  {
    id: 2,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975732/lynxmart/img/features/f1_krxbdn.png',
    title: 'Online Order',
  },
  {
    id: 3,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975732/lynxmart/img/features/f3_gcxgwq.png',
    title: 'Save Money',
  },
  {
    id: 4,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975731/lynxmart/img/features/f2_wyvv7n.png',
    title: 'Promotions',
  },
  {
    id: 5,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975728/lynxmart/img/features/f5_nnorby.png',
    title: 'Happy Sell',
  },
  {
    id: 6,
    url: 'https://res.cloudinary.com/khobbylynx/image/upload/v1683975740/lynxmart/img/features/f6_n9zobo.png',
    title: 'F24/7 Support',
  },
]

export const dropdown = [
  {
    id: 1,
    name: 'my account',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345491/lynxmart/img/icons/user-circle_m562kg.png',
  },
  {
    id: 2,
    name: 'orders',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345492/lynxmart/img/icons/orders_tnidia.png',
  },
  {
    id: 3,
    name: 'inbox',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345491/lynxmart/img/icons/envelop_su5ymo.png',
  },
  {
    id: 4,
    name: 'saved items',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345490/lynxmart/img/icons/saved_bcvkc8.png',
  },
  {
    id: 6,
    name: 'recently viewed',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345490/lynxmart/img/icons/recent_deyjjr.png',
  },
]

export const mobileMenu = [
  {
    id: 1,
    text: 'home',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688346000/lynxmart/img/icons/home-page_isvowt.png',
  },
  {
    id: 2,
    text: 'shop',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688346000/lynxmart/img/icons/online-shop_xykyon.png',
  },
  {
    id: 3,
    text: 'blog',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688346000/lynxmart/img/icons/blog_stya7d.png',
  },
  {
    id: 4,
    text: 'about',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688346000/lynxmart/img/icons/about-page_bf2jim.png',
  },
  {
    id: 5,
    text: 'contact',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688346000/lynxmart/img/icons/contact-page_cffcsq.png',
  },
]

export const mobileMenuCategory = [
  {
    id: 1,
    text: 'clothing',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345491/lynxmart/img/icons/fashion_fthyls.png',
  },
  {
    id: 2,
    text: 'phones',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345490/lynxmart/img/icons/phone_m7x4py.png',
  },
  {
    id: 3,
    text: 'computing',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345491/lynxmart/img/icons/computing_dzulzf.png',
  },
  {
    id: 4,
    text: 'health & beauty',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345491/lynxmart/img/icons/beauty_ejep6r.png',
  },
  {
    id: 5,
    text: 'electronics',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345491/lynxmart/img/icons/electronics_shervg.png',
  },
  {
    id: 6,
    text: 'sports',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345490/lynxmart/img/icons/sports_rmbih5.png',
  },
  {
    id: 7,
    text: 'Home & office',
    icon: 'https://res.cloudinary.com/khobbylynx/image/upload/v1688345491/lynxmart/img/icons/office_ghqzbu.png',
  },
]
